...
std::optional<NodeTerm *> SyntaxAnalyzer::parse_term()
{
...
std::optional<NodeExpr *> SyntaxAnalyzer::parse_expr(int min_prec)
{
...
std::optional<NodeStmt *> SyntaxAnalyzer::parse_stmt()
{
...
std::optional<NodeProg> SyntaxAnalyzer::parse_prog()
{
    NodeProg prog;
...
std::optional<Token> SyntaxAnalyzer::peek(int offset) const
{
...
Token SyntaxAnalyzer::try_consume(TokenType type, const std::string &err_msg)
{
...
std::optional<Token> SyntaxAnalyzer::try_consume(TokenType type)
{
    if (peek().has_value() && peek().value().type == type)
    {
        return consume();
